...
function loadWithdrawalOrders(username: string): WithdrawalOrder[] {
  try {
    const raw = localStorage.getItem(`bppay_withdrawal_orders_${username}`);
    return raw ? JSON.parse(raw) : [];
  } catch {
    return [];
  }
}
function saveWithdrawalOrders(username: string, orders: WithdrawalOrder[]) {
  localStorage.setItem(
    `bppay_withdrawal_orders_${username}`,
...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  const [withdrawalOrders, setWithdrawalOrders] = useState<WithdrawalOrder[]>(
    () => loadWithdrawalOrders(currentUsername),
  );
  const [withdrawPopupOpen, setWithdrawPopupOpen] = useState(false);
  const [withdrawUpiInput, setWithdrawUpiInput] = useState("");
  ...
  useDashboardStats(); // kept to avoid removing the hook; data not used (per-user data in localStorage)
  ...
  // Backend data is shared/global and must NOT override per-user localStorage stats.
  // Stats are loaded from and saved to localStorage keyed by username.
  ...
  const handleConfirmWithdrawal = () => {
    const amount = Number.parseFloat(withdrawAmountInput);
    if (!withdrawUpiInput.trim()) {
    ...
    const order: WithdrawalOrder = {
      ...
      upi: withdrawUpiInput.trim(),
      amount,
      date: new Date().toLocaleString(),
      status: "success",
    };
    const updated = [order, ...withdrawalOrders];
    setWithdrawalOrders(updated);
    saveWithdrawalOrders(currentUsername, updated);
    setWithdrawPopupOpen(false);
    toast.success(
      `Withdrawal successful! Money sent to ${withdrawUpiInput.trim()}`,
    );
  };

  const handleEditBalance = () => {
    const val = Number.parseFloat(editBalanceValue);
    ...
    setLocalStats((prev) => ({
      ...prev,
      totalCurrentBalance: val,
      lockedDeposit: val,
    }));
    setEditBalanceOpen(false);
    setEditBalanceValue("");
    toast.success(`Balance updated to ₹${val.toFixed(2)}`);
  };
  ...
  return (
    ...
    >
      <div className="w-full max-w-[420px] flex flex-col pb-10">
                ...
                <div className="flex-1">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      onClick={() => {
                        setEditBalanceValue(
                          localStats.totalCurrentBalance.toFixed(2),
                        );
                        setEditBalanceOpen(true);
                      }}
                      className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold mb-2"
                      style={{
          ...
          >
            {[
              ...
              },
              {
                ...
                action: () => {
                  if (isAdmin || withdrawReady) {
                    const targetUpi =
                      entries.length > 0 ? entries[0].upiId : "";
                    setWithdrawUpiInput(targetUpi);
        ...
        {withdrawalOrders.length > 0 && (
          <motion.div
            ...
            className="rounded-2xl overflow-hidden mx-4 mb-4"
            style={goldenCard}
          >
            <div className="px-4 pt-4 pb-2 flex items-center justify-between">
              <h2 className="text-sm font-bold" style={{ color: "#1565C0" }}>
              ...
              <span className="text-gray-400 text-[11px]">
                {withdrawalOrders.length} orders
              </span>
            </div>
      ...
      {/* Withdrawal Popup */}
      <Dialog open={withdrawPopupOpen} onOpenChange={setWithdrawPopupOpen}>
        <DialogContent
        ...
        >
          ...
          <DialogFooter className="gap-2">
            <Button
              ...
              onClick={() => setWithdrawPopupOpen(false)}
          ...
          <div className="flex flex-col gap-4 py-2">
            ...
            >
              <p className="text-xs text-amber-700 font-semibold">
                ⚠️ Admin only — changes are saved immediately
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
              <Label className="text-xs font-semibold text-gray-600">
              ...
              <Input
                ...
                className="rounded-xl text-sm text-gray-800"
                style={{ background: "#f8fafc", border: "1px solid #e2e8f0" }}
                onKeyDown={(e) => e.key === "Enter" && handleEditBalance()}
